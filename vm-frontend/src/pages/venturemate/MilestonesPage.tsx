        ...
        {milestones.length > 0 ? milestones.map((milestone) => (
          <Card key={milestone.id} sx={{ bgcolor: 'var(--vm-bg-secondary)', border: '1px solid var(--vm-border-subtle)', borderRadius: 3, p: { xs: 2, md: 3 } }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
              <Chip size="small" label={milestone.status} sx={{ bgcolor: `${getStatusColor(milestone.status)}20`, color: getStatusColor(milestone.status), fontWeight: 600, textTransform: 'capitalize' }} />
              <Chip size="small" label={milestone.priority} sx={{ bgcolor: `${getPriorityColor(milestone.priority)}20`, color: getPriorityColor(milestone.priority), fontWeight: 600, textTransform: 'capitalize' }} />
              <Chip size="small" label={milestone.category} sx={{ bgcolor: 'var(--vm-bg-tertiary)', color: 'var(--vm-text-secondary)', fontWeight: 600, textTransform: 'capitalize' }} />
            </Box>
