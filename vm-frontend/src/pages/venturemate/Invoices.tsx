...
import { Box, Typography, Card, Chip, Avatar, Dialog, DialogTitle, DialogContent, TextField, IconButton, Tooltip, CircularProgress, Button, Select, MenuItem, FormControl, InputLabel } from '@mui/material';
...
export function InvoicesPage() {
  ...
  return (
    <Box sx={{ p: { xs: 1.5, sm: 2, md: 3 } }}>
      ...
      {loading ? <Box sx={{ textAlign: 'center', py: 6 }}><CircularProgress size={20} sx={{ color: 'var(--vm-primary-400)' }} /></Box> : invoices.length === 0 ? (
      ...
      <Dialog open={!!form} onClose={() => setForm(null)} maxWidth="sm" fullWidth
        ...
        <DialogTitle sx={{ borderBottom: '1px solid var(--vm-border-subtle)', display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Typography sx={{ fontWeight: 700 }}>New Invoice</Typography>
