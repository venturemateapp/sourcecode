...
func init() {
	...
	rootMutation.AddFieldConfig("updateBusiness", &graphql.Field{
		...
		Resolve: func(p graphql.ResolveParams) (interface{}, error) {
			...
			if v, ok := p.Args["name"]; ok && v != nil { existing.Name = v.(string) }
			if v, ok := p.Args["tagline"]; ok && v != nil { existing.Tagline = v.(string) }
			...
			if v, ok := p.Args["brandKit"]; ok && v != nil { existing.BrandKit = v.(string) }
			if v, ok := p.Args["pitchDeck"]; ok && v != nil { existing.PitchDeck = v.(string) }
			if v, ok := p.Args["businessPlan"]; ok && v != nil { existing.BusinessPlan = v.(string) }
			if v, ok := p.Args["milestones"]; ok && v != nil { existing.Milestones = v.(string) }
			if v, ok := p.Args["team"]; ok && v != nil { existing.Team = v.(string) }
			if v, ok := p.Args["documents"]; ok && v != nil { existing.Documents = v.(string) }
			if v, ok := p.Args["websiteConfig"]; ok && v != nil { existing.WebsiteConfig = v.(string) }
			if v, ok := p.Args["financials"]; ok && v != nil { existing.Financials = v.(string) }
			if v, ok := p.Args["metrics"]; ok && v != nil { existing.Metrics = v.(string) }
			if v, ok := p.Args["aiGenerated"]; ok && v != nil { existing.AIGenerated = v.(string) }
